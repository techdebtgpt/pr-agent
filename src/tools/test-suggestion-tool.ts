 ...
 */
export function detectTestFramework(repoPath: string = '.'): {
 ...
 */
export function suggestTestFilePath(sourceFilePath: string, framework: string): string {
    ...
    return new DynamicStructuredTool({
        ...
        func: async ({ files, framework: providedFramework, repoPath }) => {
            ...
            for (const file of codeFiles) {
                ...
                const hasPRTest = testFilesInPR.some(testPath =>
                    testPath.toLowerCase().includes(baseNameWithoutExt.toLowerCase())
                );
