...
import { GitHubAPIError, GitError, ConfigurationError } from './errors.js';
 ...
 */
async function getDefaultBranchFromGitHub(
  owner: string,
  repo: string,
  token?: string,
): Promise<string | null> {
  ...
  try {
    ...
    if (error.status === 404) {
      throw new GitHubAPIError(
        `Repository ${owner}/${repo} not found. Check repository name and access permissions.`,
        404,
        error,
      );
      ...
      throw new GitHubAPIError(
        `Failed to fetch repository info from GitHub: ${error.message}`,
        error.status,
        error,
      );
    }
 ...
 */
function getDefaultBranchFromGit(): string | null {
  try {
...
export async function resolveDefaultBranch(
  options: BranchResolutionOptions = {},
    ...
    try {
      ...
      if (!fallbackToGit) {
        throw new ConfigurationError(
          `Configured branch "${configBranch}" not found. Run: pr-agent config --set git.defaultBranch=<branch-name>`,
          'git.defaultBranch',
        );
      }
    }
  ...
  if (githubToken) {
    ...
    if (repoInfo) {
      try {
        const defaultBranch = await getDefaultBranchFromGitHub(
          repoInfo.owner,
          repoInfo.name,
          githubToken,
        );
