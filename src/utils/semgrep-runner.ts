 ...
 */
export function getSemgrepRulesets(language?: string, framework?: string): string[] {
  const rulesets = new Set<string>(['auto', 'p/security-audit', 'p/owasp-top-ten']);
 ...
 */
export function summarizeSemgrepFindings(result: SemgrepResult): SemgrepSummary {
  ...
  const criticalFindings: SemgrepFinding[] = [];
 ...
 */
export function formatSemgrepFinding(finding: SemgrepFinding): string {
  const severity = finding.extra.severity;
