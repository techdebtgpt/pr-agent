 ...
 */
export function detectCoverageTool(repoPath = '.') {
    ...
    if (fs.existsSync(packageJsonPath)) {
        try {
            ...
            const hasCoverageScript = Object.values(scripts).some((script) => typeof script === 'string' && (script.includes('--coverage') || script.includes('nyc')));
            if (hasCoverageScript) {
                // Determine which tool
                const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
                if (deps.jest)
                    return { tool: 'jest', configured: true };
                if (deps.nyc || deps['istanbul'])
                    return { tool: 'nyc', configured: true };
                if (deps.vitest)
                    return { tool: 'vitest', configured: true };
                if (deps.c8)
                    return { tool: 'c8', configured: true };
 ...
 */
export function findCoverageFiles(repoPath = '.') {
    const foundFiles = [];
 ...
 */
function parseJestCoverageSummary(filePath) {
    try {
